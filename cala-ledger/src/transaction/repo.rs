...
use super::{entity::*, error::TransactionError, TransactionsByAccountIdCursor, TransactionsByAccountSetIdCursor};
...
pub async fn find_by_account_set_id(
    ...
    args: PaginatedQueryArgs<TransactionsByAccountSetIdCursor>,
) -> Result<PaginatedQueryRet<Transaction, TransactionsByAccountSetIdCursor>, TransactionError> {
    // This implementation follows the brokerage model transition
    // Supporting both internal ledger entries and external market data
    
    // First, find transaction IDs related to the given account set ID from the ledger
    let tx_ids = if let Some(cursor) = &args.after {
        // With pagination cursor
        sqlx::query_scalar::<_, TransactionId>(
            r#"
    ...
    for id in tx_ids {
        ...
        match self.find_by_id(id).await {
            Ok(entity) => {
                ...
                let cursor = TransactionsByAccountSetIdCursor::from((&entity, account_set_id));
                end_cursor = Some(cursor.clone());
                entities.push(entity);
            },
            Err(TransactionError::CouldNotFindById(_)) => {
